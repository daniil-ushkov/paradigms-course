...
public class Add<T extends Number> extends BinaryOperation<T> implements Assosiative {
    public Add(TripleExpression<T> el1, TripleExpression<T> el2, Operator<T> op) {
        super(el1, el2, "+", op);
    }

    @Override
    protected T calculate(T first, T second) {
        return op.sum(first, second);
    }

    @Override
    protected void throwExceptions(T first, T second) throws ExpressionException {
        op.throwSum(first, second, this);
    }
